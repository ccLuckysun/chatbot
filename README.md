...
一个最小但完整的中文 RAG 问答助手。项目主链路是：

```text
文档加载/切片 -> Embedding 模型 -> ChromaDB 向量数据库 -> 检索器 -> 上下文构建 -> LLM -> 前端展示答案与来源
```
...
- 标准库 `http.server` 提供网页聊天界面和 JSON API。
- 支持读取 `data/documents/` 下的 `.md` 与 `.txt` 文档。
- 使用 OpenAI-compatible Embedding API 生成文档向量。
- 使用 ChromaDB 本地持久化向量库，默认目录是 `vector_store/`。
- 使用向量检索结果构建上下文，再调用 OpenAI-compatible LLM 生成回答。
...
```powershell
python --version
python -c "import openai, chromadb, dotenv; print('dependencies ok')"
```
...
ChromaDB 向量库重建注意事项：同一时间只运行一个 `python app.py`。如果开了多个服务进程，它们会共享同一个 `vector_store/` 目录；一个进程重建索引时，另一个进程可能仍持有旧的 collection 引用，导致查询时报错。
...
遇到这个错误时，先停止所有正在运行的 `python app.py`，再重新启动一个服务进程。当前代码已经改为清空 collection 内文档，而不是删除整个 collection，以减少这个问题再次发生。
...
完整配置后，应用启动时会读取 `data/documents/`、生成 embeddings 并写入 ChromaDB。添加或修改知识库文档后，在页面侧边栏点击“重建知识库索引”。
...
- 页面显示本地兜底模式：说明 `.env` 没有完整配置 `LLM_*` 和 `EMBEDDING_*`，或者仍在使用 `your_*` / `api.example.com` 这类模板占位值。修改 `.env` 后需要停止并重新执行 `python app.py`。
- 修改 `.env` 后页面仍然不变：浏览器连接的可能是旧服务进程。先停止正在运行的 `python app.py`，再重新启动；必要时检查 8000 端口是否仍被旧进程占用。
